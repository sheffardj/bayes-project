...
cases %>% 
  filter(fixed=='mu') %>% 
  group_by(mu, sigma) %>% 
  slice_min(order_by = kl, n=1) %>% 
  arrange(mu, desc(sigma)) -> best_cases
...
if(1){
  ...
  for(ii in c(0,1,2)){
    for(jj in c(3.16, 1.78, 0.32)){
      ...
      best_est <- round(group_priors[which.min(group_priors$kl),"best_est"],3)
