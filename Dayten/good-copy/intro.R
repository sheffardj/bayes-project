...
parms <- matrix(
  c(0, 3.16, 0, 1.78, 0, .32, 1, 3.16, 1, 1.78, 1, 0.32),
...
if(1){ # DONT RUN THIS IF YOU DONT NEED TO
  ...
  colnames(dat) <- c("dat", "group")
  dat <- dat %>% as.data.frame() %>% mutate_at("dat", as.numeric)
  ...
  for (ii in 1:dim(parms)[1]) {
    ...
    bins <- rep(c(2,0.175,0.2),2)*sqrt(n)
...
ggarrange(do.call("grid.arrange", c(plots_list, ncol=3)), nrow=2, ncol=3)
ggsave(
  "cases.png",
  last_plot(),
  device = "png",
