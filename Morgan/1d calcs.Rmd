...
g <-ggplot(as.data.frame(ex), aes(x=x)) + 
  ...
  scale_color_manual(values = c('est curve' ='red','true curve'='black')) 
...
```{r}
s5<-est.calc( un, 1.78,2, 'D) Uniform prior, sigma = 1.78')
s6<-est.calc( nor, 1.78, 2, 'E) Norm prior, sigma = 1.78')
# s7<-est.calc( bet, 1.78)
s8<-est.calc(sn, 1.78, 2, 'F) Skewnorm prior, sigma = 1.78')
...
s9<-est.calc( un, .32, 5, 'G) Uniform prior, sigma = 0.32')
s10<-est.calc( nor, .32, 5,'H) Norm prior, sigma = 0.32')
# s11<-est.calc( bet, .32)
s12<-est.calc( sn, .32, 5, 'I) Skewnrom prior, sigma = 0.32')
